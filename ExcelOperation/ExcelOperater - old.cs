...
namespace TestItemStatisticsAcync.ExcelOperation
{
    ...
    internal class ExcelOperater
    {
        ...
        // 从测试项中提取数据
        public async Task ExtractDataFromTestItem(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
            ...
            numSN += 2;// 添加标题行和空行
            int num = 0;// row count
            string sourceWorkbookPath = ParamTestItem.SourcePath;

            await Task.Run(() => { /* 空操作 */ }); // 使用 Task.Run 来启动一个无操作的异步任务
            try
            {
                if (!CheckFilesOk(LogMsg, sourceWorkbookPath))
                    return;
                ExcelPackage.LicenseContext = LicenseContext.NonCommercial;// 设置 EPPlus 许可证上下文                                                                  
                FileInfo workbookPath = new FileInfo(sourceWorkbookPath);// 打开工作簿
            ...
            LogMsg.Message += $"item count: {count}\r\n";
            LogMsg.Message += "测试项数据 提取 完成！------------------------------\r\n";
        }
        //将提取数据拷贝粘贴到GRR module
        public async Task PasteToGRRModuleFromExtractData(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
            ...
            try
            {
                if (!CheckFilesOk(LogMsg, sourceWorkbookPath, targetWorkbookPath))
                    return;
                string[] sheetName = await GetSheetName(targetWorkbookPath).ConfigureAwait(false);//get target sheet nam
                ...
                using (ExcelPackage destPackage = new ExcelPackage(destinationFile)) // 打开目标文件
                {
                    ...
                    await Task.Run(() =>
                    {
                        ...
                        LogMsg.Message += $"item count: {sheetName.Length}\r\n";
                    });
                }
            }
            catch (Exception ex)
        ...
        public async Task PasteToGRRModuleFromLimit(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
            int stRow = ParamTestItem.StartRow;//数据源行开始:2
            int stCol = ParamTestItem.StartCol;//数据源列开始:3
            int stRowDest = ParamTestItem.StartRowDest;//目标行开始:9
            ...
            try
            {
                ...
                string[] sheetName = await GetSheetName(targetWorkbookPath).ConfigureAwait(false);//get target sheet name
        ...
        //将提取数据拷贝粘贴到GRR module
        public async Task PasteToGRRModuleForSummaryFormula(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
            string[] labels = { "LowLimit", "HighLimit", "CP", "CPK", "GRR Value" };
            int[] rowDests = ParamTestItem.CellParams.ElementAtOrDefault(0);// 目标行
            int[] colDests = ParamTestItem.CellParams.ElementAtOrDefault(1);// labels 列序号
            string targetSheet = ParamTestItem.ToSheet;
            string targetWorkbookPath = ParamTestItem.TargetPath;
            try
            {
                ...
                using (ExcelPackage destPackage = new ExcelPackage(destinationFile)) // 打开目标文件
                {
                    ...
                    LogMsg.Message += $"Count{string.Empty,-34},Summary工作表\r\n";
                    await Task.Run(() =>
                    {
        ...
        internal async Task CreatSheet(ParametersTestItem ParamTestItem, LogMessage LogMsg, bool before = false)
        {
            try
            {
                ...
                //var sheetNa = await GetSheetName(targetWorkbookPath).ConfigureAwait(false);
        ...
        // rename sheet
        internal async Task RenameSheet(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
            try
            {
                string targetWorkbookPath = ParamTestItem.TargetPath;
                ...
                string[] oldSheetName = await GetSheetName(targetWorkbookPath).ConfigureAwait(false);
        ...
        //删除range单元格
        internal async Task DeleteRangeData(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
        ...
        //复制粘贴range单元格
        internal async Task CopyRangePaste(ParametersTestItem ParamTestItem, LogMessage LogMsg)
        {
        ...
        internal async Task<string[]> GetSheetName(string excelFilePaht, bool allSheet = false, string outputFilePath = null)
        {
            ...
            try
            {
                ...
                if (!CheckFilesOk(LogMsg, excelFilePaht))
                    return sheetNames;
        ...
        private static bool IsFileLocked(string filePath)
        {
            ...
            try
            {
                stream = file.Open(FileMode.Open, FileAccess.ReadWrite, FileShare.None);// 尝试打开文件流以共享读写模式
            }
            catch (IOException)
            {
                return true;// 如果捕获到异常，文件被锁定
            }
            finally
            {
                stream?.Close();
        ...
        private bool CheckFilesOk(LogMessage LogMsg, string filePath1 = null, string filePath2 = null)
        {
            bool resultOk = true;
            ...
            void CheckFile(string filePath)
            {
                ...
                {
                    LogMsg.Message += $"文件：{filePath} 不存在\r\n";
                    resultOk = false;
                }
                else if (IsFileLocked(filePath))
                {
                    LogMsg.Message += $"文件已被打开，请关闭文件: {filePath}\r\n";
                    resultOk = false;
                }
            }
            if (filePath1 != null) CheckFile(filePath1);
            if (filePath2 != null) CheckFile(filePath2);
